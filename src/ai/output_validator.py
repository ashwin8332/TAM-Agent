...
def validate_and_repair(
    ...
    # Urgency must be valid enum (category is soft-checked)
    if parsed.get("urgency_tier") not in VALID_URGENCIES:
        return parsed, False, f"Invalid urgency_tier: {parsed.get('urgency_tier')!r}"

    # Log soft warnings for out-of-enum category/product (don't fail)
    if parsed.get("issue_category") not in VALID_CATEGORIES:
        logger.warning("Unrecognised issue_category", extra={"got": parsed.get("issue_category")})
